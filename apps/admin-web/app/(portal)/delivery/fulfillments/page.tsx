...
function FulfillmentQueue() {
  ...
  const columns: Column<FulfillmentListItem>[] = [
    {
      ...
      header: "Fulfillment",
      render: (row) => (
        <Link href={`/delivery/fulfillments/${row.id}`} className="table-link">
          {row.id.slice(0, 8)}…
        </Link>
      ),
    },
  ...
  if (!canRead) {
    return (
      <div>
        <PageHeader title="Fulfillments" subtitle="delivery.read required" />
  ...
  return (
    <div>
      <PageHeader
        title="Fulfillment cases"
        subtitle="A10 handoff / carrier cases (legacy logistics queue)."
      />
